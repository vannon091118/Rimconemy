...
namespace Rimconemy.ScavengerInfrastructure.Tests
{
    ...
    public static class StainlessSteelChainRegressionTests
    {
        ...
        private static void TestS1_StainlessSteelDefExists()
        {
            ...
            ts.Check(def != null, "S1.Rimconemy_StainlessSteel ThingDef is loaded");
            if (def != null)
            {
                ts.Check(def.stackLimit > 0, "S1.StainlessSteel has valid stackLimit");
            }
        }
        ...
        private static void TestS2_MakeStainlessSteelRecipeExists()
        {
            ...
            ts.Check(recipe != null, "S2.Rimconemy_MakeStainlessSteel RecipeDef is loaded");
            if (recipe != null)
            {
                // Check products: 2 StainlessSteel
                ts.Check(recipe.products != null && recipe.products.Count > 0, "S2.MakeStainlessSteel has products");
                bool producesSS = recipe.products.Exists(p =>
                    p.thingDef != null && p.thingDef.defName == "Rimconemy_StainlessSteel" && p.count == 2);
                ts.Check(producesSS, "S2.MakeStainlessSteel produces 2 Rimconemy_StainlessSteel");
            }
        }
        ...
        private static void TestS3_StainlessSteelTowerDefExists()
        {
            ...
            ts.Check(def != null, "S3.Rimconemy_StainlessSteelTower ThingDef is loaded");
            if (def != null)
            {
                ts.Check(def.thingClass != null && def.thingClass.Name.Contains("Turret"), "S3.Tower has Building_TurretGun class");
                ts.Check(def.size.x == 2 && def.size.z == 2, "S3.Tower size is 2x2");
            }
        }
        ...
        private static void TestS4_CampfireHasMakeStainlessSteel()
        {
            ...
            ts.Check(campfire != null, "S4.Rimconemy_Campfire ThingDef is loaded");
            if (campfire != null)
            {
                ts.Check(campfire.recipes != null && campfire.recipes.Count >= 4, "S4.Campfire has at least 4 recipes");
                bool hasMakeSS = campfire.recipes.Exists(r => r != null && r.defName == "Rimconemy_MakeStainlessSteel");
                ts.Check(hasMakeSS, "S4.Campfire has MakeStainlessSteel");
            }
        }
        ...
        {
            var def = DefDatabase<ThingDef>.GetNamedSilentFail("Rimconemy_StainlessSteel");
            ts.Check(def != null, "S5.Rimconemy_StainlessSteel ThingDef is loaded");
            if (def != null && def.stuffProps != null)
            {
                ts.Check(def.stuffProps.categories != null && def.stuffProps.categories.Any(c => c.defName == "Metallic"), "S5.StainlessSteel has Metallic stuff category");
                ...
                ts.Check(hasMaxHPFactor, "S5.StainlessSteel has MaxHitPoints factor > 1.2");
            }
            else
            {
                ts.Check(false, "S5.StainlessSteel missing stuffProps");
            }
        }
        ...
        private static void TestS6_StainlessSteelCategory()
        {
            ...
            if (def != null && def.thingCategories != null)
            {
                ...
                ts.Check(hasCat, "S6.StainlessSteel has Scraps category");
            }
            else
            {
                ts.Check(false, "S6.StainlessSteel category check failed");
            }
        }
        ...
        private static void TestS7_TowerRequiresPower()
        {
            ...
            if (def != null && def.comps != null)
            {
                ...
                ts.Check(powerComps.Count > 0, "S7.Tower has CompPowerTrader comp");
                if (powerComps.Count > 0)
                {
                    ...
                    ts.Check(powerConsumption == 150f, "S7.Tower power consumption is 150W");
                }
            }
            else
            {
                ts.Check(false, "S7.Tower comps check failed");
            }
        }
        ...
        private static void TestS8_TowerCost()
        {
            ...
            if (def != null && def.costList != null)
            {
                ...
                ts.Check(hasSS, "S8.Tower cost includes 30 StainlessSteel");
                ts.Check(hasParts, "S8.Tower cost includes 4 MachineParts");
                ts.Check(hasSteel, "S8.Tower cost includes 20 Steel");
                ts.Check(hasWC, "S8.Tower cost includes WeaponComponent (D3-Harmonisierung)");
            }
            else
            {
                Log.Error("[Rimconemy.ScavengerInfrastructure] S8.Tower costList check failed - def or costList null");
                ts.Check(false, "S8.Tower costList check failed");
            }
        }
        ...
        private static void TestS9_MakeStainlessHasResearchGate()
        {
            ...
            ts.Check(recipe.researchPrerequisites != null && recipe.researchPrerequisites.Count > 0, "S9.MakeStainlessSteel has researchPrerequisites (Phase-First)");
        }
