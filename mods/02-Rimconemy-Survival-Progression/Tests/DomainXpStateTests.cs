...
namespace Rimconemy.SurvivalProgression.Tests
{
    ...
    public static class DomainXpStateTests
    {
        ...
        public static bool RunAll()
        {
            ...
            AssertEqual(2, empty.GetLevel(ProgressionDomain.Building), "level crossed to 2 (>=100 xp)");
            ...
            // 5) Diminishing returns — keys must carry the domain prefix or
            // AwardCountByDomain returns 0 and all factors collapse to 1.0.
            var dim = new DomainXpState();
            const int n = 5;
            string prefix = DomainXpState.BuildDomainActionPrefix(ProgressionDomain.Building);
            for (int i = 0; i < n; i++)
            {
                dim.TryAward(
                    ProgressionDomain.Building, 10f, prefix + "dim:" + i, "Def_X", 1, i, out _);
            }
            // Factor curve is (c+5)/(2c+5): c=0→1.0, c=1→0.857, c=2→0.778, c=3→0.727, c=4→0.692.
            // Total xp is below 5×10=50 but well above 5×5=25 (lower bound sanity).
            ...
            // Use unprefixed keys so diminishing-returns factor stays at 1.0 for
            // every award — we only need the sqrt-based level transition tested.
