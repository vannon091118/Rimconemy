...
FALSIFICATION_DIR="$PROJECT_ROOT/docs/archive/falsification"
TEST_FILE="$TESTS_DIR/${NAME}RegressionTests.cs"
FALS_FILE="$FALSIFICATION_DIR/${PKG}__${NAME}.md"

if [[ -e "$TEST_FILE" || -e "$FALS_FILE" ]]; then
    echo "Refusing to overwrite existing scaffold output." >&2
    [[ -e "$TEST_FILE" ]] && echo "  exists: $TEST_FILE" >&2
    [[ -e "$FALS_FILE" ]] && echo "  exists: $FALS_FILE" >&2
    exit 1
fi
...
namespace Rimconemy.${NS}.Tests
{
    public static class ${NAME}RegressionTests
    {
        ...
        public static void RunAll()
        {
            ...
            // Deliberately red until a real observable check is supplied.
            // This prevents a scaffold from producing false green evidence.
            ts.Check(false, "T1.NOT_IMPLEMENTED — replace scaffold gate");
...
- [ ] Falsification gates A–G ticked off (archive evidence path)
MDHERE
...
echo ""
echo "Next steps:"
echo "  1. Edit $TEST_FILE — replace the deliberately failing scaffold gate with a real observable check"
