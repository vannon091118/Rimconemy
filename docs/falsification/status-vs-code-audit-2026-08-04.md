...
| B3 | Historischer Befund: Forschungsbaum war nur Read-Model, kein Capability-Toggle | `ProgressionGameComponent.UpdateResearchCapabilities` schreibt `ResearchCapabilities` aus `DefDatabase<ResearchProjectDef>.AllDefs`. Es gibt keinen organischen Action-Completion-/Unlock-State im aktuellen Code. | **BESTÄTIGT:** Der alte Forschungsbaum war nicht geliefert. Kanonische Folgeentscheidung: bestätigte Handlung → Erfahrung → Architektenfreigabe; Vanilla-Research bleibt Legacy-/Kompatibilitätsschicht. |
...
| C3 | `BauschuttRemapService.cs:35-87` ist nur Proposal-System, mutiert nicht | Pre-Phase-3.2: `PlanRemapForCurrentMap()` returns `RemapProposal` — reines Proposal ohne Mutation. Ab Phase-3.2 (2026-08-04): `BauschuttRemapApply.ApplyRemap()` wurde neu hinzugefügt, liest Storage-Bauschutt-Count via `StorageQuery.ReadStorage`, platziert via `GenConstruct.PlaceBlueprintForBuild` Wall-Blueprints auf Map-Edge-Cells (deterministische Scan-Reihenfolge) und fordert anschließend über `StorageWriteMutationService` einen best-effort Abzug an. Bei Teilbestand/Fehler gibt es keinen atomaren Rollback. Vanilla-Healthy-Verification der tatsächlichen Platzierung bleibt Runtime-Gate. `Designator_BuildWallBauschutt : Designator_Build` ist der vanilla-UI-Hook OHNE Harmony-Transpiler. Compile-Sicherheit: Wall-Def-Lookup via `DefDatabase<ThingDef>.GetNamedSilentFail` (vanilla-native), Stuff-Def via `DefDatabase` oder Test-Seam. Test-Harness: 7 statische Placement-Seams plus `StorageWriteMutationService.MutateDownOverride` und `ResetTestSeams()`. Production-Pfad bleibt strikt: Alle Seams default-null, kein Verhalten in Production kompromittiert. | **EINGESCHRÄNKT BESTÄTIGT** — `BauschuttRemapApply.ApplyRemapCore` ist testbar (synthetische Inputs), `ApplyRemap` ist Storage-Bridge und der Write-Hook ist verdrahtet. `Tests/BauschuttRemapApplyTests.RunAll` enthält **8 T1–T8 Assertions**; T8 prüft den Storage-Write-Vertrag. Vanilla-Mutation und End-to-end-Abzug selbst (`GenConstruct.PlaceBlueprintForBuild`/Map-Storage) bleiben Runtime-Gate; Partial-Removal ist möglich. |
...
| C7 | Owner-Constraint Paket 03 ↔ Pakete 02/05 (Code-Seite) | Paket 03 exportiert `rimconemy.scavengerinfrastructure.resources` (Storage) und `rimconemy.scavengerinfrastructure.power` (Power-Chain) als Capability; Reader (02/05) gehen via `CapabilityAudit.HasCapabilityOrWarn(...)`. Ab Phase-3.2 (2026-08-04): `BauschuttRemapApply.ApplyRemap` als erste reale Map-Mutation (Wall-Blueprints via `GenConstruct.PlaceBlueprintForBuild`) plus best-effort Storage-Write im selben Apply-Pfad. Owner-Constraint bleibt hart: Paket 03 mutiert NUR Map-Blueprints/Buildings und den eigenen physischen Storage-Bestand, NICHT Wallet (Paket 04). Storage-Read bleibt read-only `StorageQuery`. TargetMap-Null-Bypass nur wenn `CandidateCellOverride` Test-Seam aktiv ist, Production-Pfad strikt. | **BESTÄTIGT mit Einschränkung** im Read-/Write-Pfad: Vanilla-Mutation und Storage-Write sind verdrahtet, Test-Seams default-null und kein Cross-Package-Wallet-Schreibzugriff sichtbar. Teilabzug ohne atomaren Rollback sowie fehlender Runtime-Beleg bleiben offen. |
...
| E2 | `InfectedRaidWorker` sendet eine Letter und besitzt inzwischen eine begrenzte Spawn-Bridge | `InfectedRaidWorker.TryExecuteWorker` sendet `Find.LetterStack.ReceiveLetter(...)`, baut danach einen threat-basierten `SpawnPlan` und fordert maximal einen `Rimconemy_InfectedRavager` an; `HiddenInfected.xml` und `InfectedRavager.xml` liefern die DEF-Basis. Der tatsächliche Ingame-Spawn-/Kampflauf ist noch nicht live belegt. | **EINGESCHRÄNKT BESTÄTIGT** — Letter und begrenzte Spawn-Bridge sind CODE/DEF; vollständiger Raid bleibt OPEN. |
| E3 | `InfectedRaidSpawnService` bleibt ein SpawnPlan, der Worker führt die Spawn-Bridge aus | `InfectedRaidSpawnService.BuildPlanForTick(long tick)` liefert nur den pressure-basierten `SpawnPlan`; `InfectedRaidWorker` übernimmt danach Position/Def/Faction und den begrenzten Spawn-Versuch. | **BESTÄTIGT mit Einschränkung** — Plan und Worker-Brücke sind getrennt; vollständige Skalierung/Auflösung bleibt OPEN. |
...
| E5 | `StoryDirector` ist LiveRuntime; feuert Letter- und begrenzten Spawn-Pfad | `StoryDirector.GameComponentTick` evaluiert alle 60.000 Ticks, wählt Event und queued via `Find.Storyteller.TryFire`; `InfectedRaidWorker.TryExecuteWorker` sendet Letter und versucht danach die begrenzte Spawn-Bridge. **Letter-Pfad ist belegt; echter Pawn-Spawn/Kampflauf bleibt live unbestätigt.** | **BESTÄTIGT** für Story-/Letter-Pfad, **EINGESCHRÄNKT** für Spawn; vollständige Raids bleiben OPEN. |
...
- **Stand-Update:** B6 Save-Migration bleibt **BESTÄTIGT**; C3/C7 Bauschutt-Bau und Storage-Write sind **CODE-seitig verdrahtet**, aber ohne atomaren Rollback und ohne LIVE-Beleg.
- **WIDERLEGT:** B3 Tier-Forschung bleibt widerlegt, weil der Code nur ein Read-Model belegt.
- **EINGESCHRÄNKT/OPEN:** B2 Live-XP-Yield, E2/E3/E5 vollständige Infected-Raid-Ausführung sowie G1–G3 Performance bleiben eingeschränkt oder offen.
...
- Hat **keinen** vollständigen Paket-03 Bauschutt-Bau-/Storage-Lifecycle (Task 3.2) live belegt; der CODE-Pfad und der best-effort Write-Hook sind inzwischen vorhanden.
- Hat **keinen** vollständigen Paket-05 Raid-/Kampflauf (Task 5.4) live belegt; eine auf einen Pawn begrenzte Spawn-Bridge ist inzwischen als CODE/DEF vorhanden.
