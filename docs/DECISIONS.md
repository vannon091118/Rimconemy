...
| 2026-08-05 | **Phase B Decision (DECISION-B-001):** Day-Tick-Reihenfolge **ApplyDailyGrowthTick + ResetDailyCounters NACH EvaluateWithSnapshot**, VOR Inoculation. Bewusste Abweichung von Phase A spec §3 “Growth vor Eval”. Begründung: heute läuft das Eval mit gestrigem Cap, morgen mit neuem. Rage-Quote wird durch den Recompute am Ende des Day-Blocks gesetzt, sodass das nächste Eval den Slot lesen kann. Transient-State (kein Scribe) für `LastPendingRevenge`/`LastRevengeRefreshTick`; Schema-Stabilität; Save/Load-rebuild aus ledger.RecentKillsToday × Profile.RevengeRatio. ProfileId-Translation Layer (StripRimconemyPrefix) als Brücke zwischen `SettingProfile.ProfileId=“Rimconemy_Survival”` und `PopulationProfileMultipliers.RevengeRatio[“Survival”]` dokumentiert. Phase-A-spec §3 wird obsolet, Phase-B-spec §3 ist Kanon. |
| 2026-08-05 | **Phase E Cleanup-Pass:** (1) **DECISION-E-001 — Dual-Counter Accounting.** `CumulativeInoculations` (Phase C, lifetime-counter, alle Conversion-Pfade) und `AnimalInfectionCountToday` (Phase E, daily-counter, Tages-Reset via Math.DivRem) sind bewusst DISTINCT. `RandomInoculationService.TryInfectWildAnimals` ruft `ApplyLiveConversion` (welches `NoteInoculation` triggert) UND der AnimalInfectionDriver ruft `RegisterAnimalInfection` — beide Counter sind semantisch unterschiedlich (`LifetimeInoc` vs `TodayAnim`), nicht „doppelt gezählt“. Doc-Verweis jetzt im Ledger inline. (2) **DECISION-E-002 — Driver-Static-Mutability.** `AnimalInfectionDriver._lastFireTick` ist static für Test-Seam-Sichtbarkeit; in Production läuft der Driver als MapComponent pro Map, daher ist die Static-Variable pro Map-Instanz ok. Defensiv: statisches Reset in Tests Sorgen für Idempotenz. (3) **DECISION-E-003 — Hediff-Application-Site.** Tiersym-Aggression-Hediff wird beim Live-Conversion angewendet (in `ApplyLiveConversion`), NICHT im späteren Tick. Begründung: Pawn-Hediffs sind Pawn-Scribe-State; einmal-apply ist idempotent via `GetFirstHediffOfDef`-Guard. Live-Test-Trigger für neue Tiere bleibt simpel. | Buffy |
| 2026-08-05 | **Cycle-Break (DECISION-D-001):** Diagnose: `mods/02-Rimconemy-Survival-Progression/About/About.xml` listete `rimconemy.infectedautomation` in `<loadAfter>`, **und** `mods/05-Rimconemy-Infected-Automation/About/About.xml` listete `rimconemy.survivalprogression` — **About.xml-Edge-Cycle 02 ↔ 05**, obwohl Code (DLL-Refs) bereits cycle-frei ist (Grep `Rimconemy.SurvivalProgression.…` in 05/ = 0 Treffer; Grep `Rimconemy.InfectedAutomation.…` in 02/ = 0). **Fix:** beide Cross-Package-loadAfter-Edges entfernt; nur `rimconemy.foundation` bleibt in jedem Mod (außer 01 mit Hard-Requires). Cross-Package-Crossings (02↔05 GameOver, 05↔04 Wallet, 05↔03 Storage) laufen schon alle über `Rimconemy.Foundation.CrossPackage.CrossPackageState` mit Capability-Gate zur **Tick-Laufzeit**, nicht zur Mod-Load-Zeit; loadAfter-Reihenfolge ist modübergreifend irrelevant. INTERFACE_CONTRACT.md §9.3 präzisiert: Code UND About.xml cycle-frei; vorher war der Code-Cycle-Claim richtig aber die About.xml-Disziplin nicht explizit. Pattern ist jetzt: nur Foundation-loadAfter, alles andere via Bridge. | Buffy |
